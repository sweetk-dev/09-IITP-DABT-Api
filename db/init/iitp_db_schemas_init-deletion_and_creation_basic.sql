...
-- ## ver 0.0.9 last update data : 2025.06.23
-- ## Only for PostgreSQL
-- ## Except "mv_poi" table
...
CREATE TABLE public.sys_stats_src_api_info (
	...
	need_opt_url_yn char(1) NOT NULL, -- 추가 옵션 url 필요 여부(Y/N). use , need 모두 "Y" 이면 ext_url + opt(data, meta)  url 사용
	api_data_url varchar(600) NOT NULL, -- 연동할 stas data API URL, {API_AUTH_KEY}를 실제 Key로 치환 
	api_meta_url varchar(300) , -- 연동할 stat meta API URL, {API_AUTH_KEY}를 실제 Key로 치환  
...
COMMENT ON COLUMN public.sys_stats_src_api_info.need_opt_url_yn IS '기본 url 필요 여부(Y/N). "Y" 이면 이면 sys_ext_api_info.ext_url 사용';
COMMENT ON COLUMN public.sys_stats_src_api_info.need_opt_url_yn IS '추가 옵션 url 필요 여부(Y/N). use , need 모두 "Y" 이면 ext_url + opt(data, meta)  url 사용';
COMMENT ON COLUMN public.sys_stats_src_api_info.api_data_url IS '연동할 stas data API URL, {API_AUTH_KEY}를 실제 Key로 치환 ';
COMMENT ON COLUMN public.sys_stats_src_api_info.api_meta_url IS '연동할 stat meta API URL, {API_AUTH_KEY}를 실제 Key로 치환 ';
