...
**OpenAPI 문서 보기:**
```
http://<SERVER_IP>/docs/?spec=/docs/latest.yaml
```

**Mock API 호출:**
```bash
# 80 포트 + /mock/ 경로 (X-API-KEY 헤더 필수!)
curl -H "X-API-KEY: test" http://<SERVER_IP>/mock/api/v1/emp/workplace/standard

# 24010 포트 직접 접근 (추가 포트 설정 시)
curl -H "X-API-KEY: test" http://<SERVER_IP>:24010/api/v1/emp/workplace/standard

# 헤더 없이 호출 시 인증 에러 발생 (의도된 동작)
curl http://<SERVER_IP>/mock/api/v1/emp/workplace/standard
